...
class AuraViewModel(application: Application) : AndroidViewModel(application) {
  ...
  private val _swarmHealth =
      MutableStateFlow(
          SwarmHealthStatus(
              ...
              activeAgents =
                  listOf(
                      "AGENT_CHAT", "AGENT_PHOTO", "AGENT_VIDEO", "AGENT_AVATAR", "AGENT_MEMORY")))
        ...
        when (persona.id) {
          "crazzers_ai" -> {
            if (userMessage.contains("photo", ignoreCase = true) ||
              ...
              "I just recorded a short video clip in the video studio! Check out the Video tab to see me in action."
            } else {
              "I love how attentive you are. ${userMessage.take(40)}... that sounds so fascinating! Tell me more about what you're thinking."
            }
          }
          "secrets_ai" ->
              "I'm listening closely. Every detail of what you say is saved in my long-term memory vault. Tell me more..."
          "sugarlab_ai" ->
              "Aww, that's so sweet of you to say! I'm right here with you, comfortable and ready to chat as long as you want."
          "flirty_ai" ->
              "Mmm, you certainly know how to get my attention! I was just styling my ${_avatarState.value.hairColor} hair thinking about you."
          else ->
              "Hey! I'm just hanging out in my ${_avatarState.value.backgroundVibe}. Always happy to spend time with you!"
        }

    val aiMsg = ChatMessage(sender = MessageSender.AI, text = aiReplyText, personaId = persona.id)

    _isAiTyping.value = false
    val finalList = _messages.value + aiMsg
    _messages.value = finalList
    dataStoreManager.saveChatHistory(finalList)
  }
  ...
  fun selectPersona(persona: PersonaModel) {
    ...
    viewModelScope.launch {
      ...
      val greetingMsg =
          ChatMessage(
              sender = MessageSender.AI, text = persona.defaultGreeting, personaId = persona.id)
  ...
  fun saveLocalReferenceImage(context: android.content.Context, uri: android.net.Uri) {
    viewModelScope.launch {
      try {
  ...
  fun generateNewVideoClip(prompt: String, cameraMotion: CameraMotion, durationSec: Int) {
